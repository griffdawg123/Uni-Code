...
int add_note_to_beat(Beat beat, int octave, int key) {
    ...
    while (current != NULL) {
        // for any note in beat, if new note is equal, invalid
        if (note_compare(current, new_note) == 0) {
            return NOT_HIGHEST_NOTE;
        }
        // if last note is higher than new note, do not add
...
void add_beat_to_track(Track track, Beat beat) {
    ...
    if (current == NULL) {
        ...
        // insert new beat after currently selected beat
...
int remove_selected_beat(Track track) {
    ...
    while (current != NULL) {
        if (current == selected) {
            // if not at the start of the track
            if (prev != NULL) {
                prev->next = current->next;
            }
            // if at the start of the track
            if (current == track->head) {
                track->head = current->next;
            }
            track->current = current->next;
            free_beat(current);
            // if current was the last beat in the track, stop track
            if (track->current == NULL) {
                return TRACK_STOPPED;
            } else {
                return TRACK_PLAYING;
...
int add_musical_note_to_beat(Beat beat, char *musical_notation) {
    ...
    // number of sharps is the length of input minus two
    int sharps = strlen(musical_notation) - 2;
    key += sharps;
    // add sharps to key then format so key and octave are valid
    while (key > 11) {
        key -= 12;
        octave++;
    }
    ...
    // if current beat is empty, just add in straight away
    if (current == NULL) {
        beat->notes = new_note;
        return VALID_NOTE;
    }
    while (current != NULL) {
        // if new note is between curretn and prev, insert it
        if ((note_compare(current, new_note) == 1) && 
        ...
        current = current->next;
        if (current == NULL) {
            // if were at the end of the beat, if new note is bigger than prev, 
            // insert at the end
    ...
    // if unable to add note, delete it
    free(new_note);
    return INVALID_MUSICAL_NOTE;
...
int reverse_range(Track track, int range_length) {
    ...
    int i = 0;
    Beat current = first;
    // go to the end of the range, cut short if range_length goes past the end
    // of the track
    ...
    Beat preStart = NULL;
    current = track->head;
    // set prestart to the beat that points to first
    if (first != track->head) {
        while (current->next != first) {
            current = current->next;
        }
    ...
    current = first;
    while (prev != last) {
        // remember the next beat in the track
        next = current->next;
        // set next to the one before
        current->next = prev;
        // move pointers along
        prev = current;
        current = next;
