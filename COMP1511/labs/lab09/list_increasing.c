// A program that determines whether or not a list is increasing
// Griffin Doyle <z5311098>
// 14/04

#include <stdio.h>
#include <stdlib.h>
#include <assert.h>

struct node {
    struct node *next;
    int          data;
};

int increasing(struct node *head);
struct node *strings_to_list(int len, char *strings[]);

// DO NOT CHANGE THIS MAIN FUNCTION

int main(int argc, char *argv[]) {
    // create linked list from command line arguments
    struct node *head = strings_to_list(argc - 1, &argv[1]);

    int result = increasing(head);
    printf("%d\n", result);

    return 0;
}


// return 1 if values in a linked list are in increasing order,
// return 0, otherwise

int increasing(struct node *head) {

    // Create current and previous node
    struct node *current = head;
    // if empty list, return 1
    if (current == NULL) {
        return 1;
    }
    // create previous node
    struct node *prev = current;
    current = current->next;

    // iterate through list
    while (current != NULL) {
        // if increasing, continue
        if (current->data > prev->data) {
            prev = current;
            current = current->next;
        } else {
            // else list is not increasing
            return 0;
        }
    }
    //got to end of the loop while increasing
    return 1;
}


// DO NOT CHANGE THIS FUNCTION

// create linked list from array of strings
struct node *strings_to_list(int len, char *strings[]) {
    struct node *head = NULL;
    for (int i = len - 1; i >= 0; i = i - 1) {
        struct node *n = malloc(sizeof (struct node));
        assert(n != NULL);
        n->next = head;
        n->data = atoi(strings[i]);
        head = n;
    }
    return head;
}
