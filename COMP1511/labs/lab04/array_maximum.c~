...
int array_maximum(int size, int array[size]) {
    ...
    // iterator i will move through all of the elements in array
    while (i < size) {
        
        // if the element at position i is greater than the current max,
        // replace maximum with that element.
        if (array[i] > max) {
            max = array[i];
        }
        i++;
