...
void access_page(int virtual_page, int access_time, int n_physical_pages, struct ipt_entry *ipt) {
    ...
    if(virtualIndex >= 0) {
        printf("Time %d: virtual page %d -> physical page %d\n", access_time, virtual_page, virtualIndex);
        ipt[virtualIndex].last_access_time = access_time;
    } else if (freePage >= 0) {
        printf("Time %d: virtual page %d loaded to physical page %d\n", access_time, virtual_page, freePage);
        ipt[freePage].virtual_page = virtual_page;
        ipt[freePage].last_access_time = access_time;
    } else {
        int lowestAccess = 0;
        for (int i = 0; i < n_physical_pages; i++) {
            if ((ipt[lowestAccess].last_access_time >  ipt[i].last_access_time) && (ipt[i].virtual_page != -1)) {
                lowestAccess = i;
            }
        }
        printf("Time %d: virtual page %d  - virtual page %d evicted - loaded to physical page %d\n", access_time, virtual_page, ipt[lowestAccess].virtual_page, lowestAccess);
        ipt[lowestAccess].virtual_page = virtual_page;
        ipt[lowestAccess].last_access_time = access_time;
    }
