...
static void doTreeSearchBetween(Tree t, Node n, Record lower,
                                Record upper, List l) {
    //rotate tree until lower is root node
    //recurse through right subtree until lower is found
    if (n == NULL) {
        return;
    }
    ...
    Node curr = n;
    while (curr != NULL) {
        if (curr->left == NULL) { // we are at the left most node of the tree
            if ((t->compare(curr->rec, lower) >= 0) && t->compare(curr->rec, upper) <= 0) {
                ListAppend(l, curr->rec);
            }
            curr = curr->right;
        } else {
            Node pre = curr->left; // predecessor of curr, right most child of left subtree OR left child

            while ((pre->right != NULL) && (pre->right != curr)) {
                pre = pre->right;
            }

            if (pre->right == NULL) { // essentially a right rotation to put pre on top
                pre->right = curr;
                curr = curr->left;
            } else {
                pre->right = NULL; 
