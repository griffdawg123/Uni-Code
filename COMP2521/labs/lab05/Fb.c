...
bool FbUnfriend(Fb fb, char *name1, char *name2) {
    int id1 = nameToId(fb, name1);
    int id2 = nameToId(fb, name2);
    assert(id1 != id2); // make sure the names aren't the same
    if (fb->friends[id1][id2]) { // check that users are already friends
...
void FbFriendRecs1(Fb fb, char *name) {
    ...
    List friendList;
    // max mutual friends for n people is n-2
    // minimum mutual friends for n people is 0

    for (int mutuals = fb->numPeople; mutuals > 0; mutuals--) {
        for (int user = 0; user < fb->numPeople; user++) {
            if (user != id) {
                friendList = FbMutualFriends(fb, fb->names[user], fb->names[id]);
                int numFriends = ListSize(friendList);
                ListFree(friendList);
                if (!fb->friends[user][id] && (numFriends == mutuals)) {
                    printf("\t%-20s%4d mutual friends\n", fb->names[user], numFriends);
                }
            }
        }
    }
