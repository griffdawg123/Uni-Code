...
{
	/* 
	Check if list is empty or new number is the smallest
		insert at start

	Check each integer and comparing number to each in the list, if less than next insert else iterate through
	if we get to the end, make sure to change L->last 

	*/

	struct IntListNode *curr;
	struct IntListNode *new = newIntListNode(v);
	if ((L->first == NULL) || (L->first->data >= new->data)) {
		if (L->first == NULL) {
			L->last = new;
		}
		new->next = L->first;
